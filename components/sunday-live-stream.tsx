...
interface SundayLiveStreamProps {
  channelId: string
  fallbackVideoId?: string
}

export function SundayLiveStream({ channelId, fallbackVideoId }: SundayLiveStreamProps) {
  ...
  useEffect(() => {
    const checkIfLive = async () => {
      try {
        ...
        // Only check if it's Sunday and between 9:30 AM and 12:30 PM (service time with buffer)
        const now = new Date()
        const isSunday = now.getDay() === 0
        const hour = now.getHours()
        const isServiceTime = hour >= 9 && hour <= 12

        // If it's not Sunday during service time, don't make the API call
        if (!isSunday || !isServiceTime) {
          console.log("Not checking for livestream - not during Sunday service hours")
          setIsLive(false)
          setLoading(false)
          return
        }

        console.log("Checking for Sunday livestream...")
    ...
    // Then check every 5 minutes, but only on Sundays
